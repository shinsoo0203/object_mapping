...
class ObjectArray {
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    obj.objects.forEach((val) => {
      bufferOffset = ObjectPoint.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    for (let i = 0; i < len; ++i) {
      data.objects[i] = ObjectPoint.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static getMessageSize(object) {
    ...
    object.objects.forEach((val) => {
      length += ObjectPoint.getMessageSize(val);
    });
    return length + 4;
  }
  ...
  static Resolve(msg) {
    ...
    if (msg.objects !== undefined) {
      ...
      for (let i = 0; i < resolved.objects.length; ++i) {
        resolved.objects[i] = ObjectPoint.Resolve(msg.objects[i]);
      }
    }
    else {
      resolved.objects = []
